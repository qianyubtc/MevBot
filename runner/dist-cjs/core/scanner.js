...
const ROUTER_ABI = (0, viem_1.parseAbi)([
...
class OnChainScanner {
    ...
    async scanTopPairs(limit = 20) {
        console.log(chalk_1.default.cyan(`[Scanner] 扫描 ${this.dexName} 最新交易对...`));
        try {
            ...
            const pairs = await Promise.allSettled(indices.map((i) => this.client.readContract({
                address: this.factoryAddress,
                abi: FACTORY_ABI,
            ...
            const pairAddresses = pairs
                ...
                .map((r) => r.value);
            const results = await Promise.allSettled(pairAddresses.map((addr) => this.analyzePair(addr)));
            const tokens = results
                ...
                .slice(0, limit);
            console.log(chalk_1.default.green(`[Scanner] 扫描完成，找到 ${tokens.length} 个优质代币`));
            return tokens;
        }
        catch (err) {
    ...
    async analyzePair(pairAddress) {
        try {
            ...
            const baseReserveNum = Number((0, viem_1.formatUnits)(baseReserve, baseDecimals));
            ...
            if (liquidityUSD < 5000)
                return null;
            // Price
            const targetReserveNum = Number((0, viem_1.formatUnits)(targetReserve, Number(decimals)));
    ...
    async getMultiDexPrices(tokenAddress, routers, amountIn = BigInt(1e18)) {
        const results = await Promise.allSettled(routers.map(async ({ name, address }) => {
            ...
            const price = Number((0, viem_1.formatUnits)(amounts[1], 18));
    ...
    async getBNBPrice() {
        try {
            ...
            return Number((0, viem_1.formatUnits)(amounts[1], 18));
        }
        catch {
            return 580;
        }
...
exports.OnChainScanner = OnChainScanner;
